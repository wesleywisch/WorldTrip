import { Box, Flex, Heading, Image, Text } from "@chakra-ui/react";

type CityProps = {
  image: string;
  ImageAlt?: string;
  name: string;
  city: string;
  icon: string;
}

export function City({ image, ImageAlt = 'default', name, city, icon }: CityProps) {
 return (
  <Box borderRadius="4px" overflow="hidden">
    <Image src={image} alt={ImageAlt} h="170px" w="100%"/>
    <Flex p="6" align="center" justify="space-between" bg="white" border="1px" borderColor="yellow.300" borderTop="0">
      <Flex direction="column">
        <Heading fontSize="xl" fontWeight="500">{name}</Heading>
        <Text mt="3" fontSize="md" color="gray.500" fontWeight="500">{city}</Text>
      </Flex>
      <Image src={icon} w="30px" h="30px" borderRadius="50%" objectFit="cover"/>
    </Flex>
  </Box>
 )
}